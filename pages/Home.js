...
const HabitatRow = ({
  ...
  const basisString = scales.shortToString(basis.gold);
...
const Habitats = ({ basis }: { basis: HabitatBasisCollection }) => {
  return habitats.All.map(habitat => {
    return (
...
const Home = () => {
  return (
    <Page>
      ...
      <Habitats basis={initHabitatBasisCollection} />
    </Page>
  );
};
